...
const HeaderContentsComponent = observer(() => {
    ...
    return (
        <div>
            {store.summonerDTO?.summoner?.previousTiers && store.summonerDTO.summoner.previousTiers.map((value, index)=>{
            
            return (<TierComponent  key={index} season={value.season} tier={value.tier} tierScore={value.string} />)
            ...
            <div>(상위{store.summonerDTO?.summoner?.ladderRank?.rankPercentOfTop})</div>
